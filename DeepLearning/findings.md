...
### `exp_085`：agreement-aware rescue-only + labeled LOO（2026-04-03）
...
- 在用户回报：
 ...
 之后，可以更清楚地确认两点：
  1. rescue 机制本身是真的
  2. “整体更保守”不是正确方向
- 因此下一步不再继续统一上调 margin，而是尝试把 rescue 子集拆开：
  - `061_argmax == 067_pred`
  - `061_argmax != 067_pred`
- Jesse 与 Mila 也各自分开，形成四套门槛：
...
- 这个实验的出发点不是“再做一次更保守”
- 而是测试一个更细的假设：
  - 当 `061` 的 argmax 已经和 `067` 一致时，rescue 应该更宽松
  - 当二者身份判断冲突时，rescue 应该更严格
- 这是对前面分析的直接落实：
  - **按样本类型决定怎么救**
  - 而不是继续用单一全局 margin 调整个系统

#### 本地结果

- 新脚本：`scripts/predict_061_067_agreement_rescue_labeledloo.py`
- `exp_085` 最优参数：
  - `low_conf_threshold = 0.45`
...
- **LOO**：
  ...
  - `fused = 0.9625`
- 也就是说：
  - 它在离线最优值上**没有超过** `exp_083 / exp_084`
  - 仍然落在同一个 plateau 上

#### 真正有信息量的地方在 test 侧

- `exp_085` 相对 `exp_061` 改动 **95** 张：
  - `0 -> 1`: `51`
  - `0 -> 2`: `44`
- 它和当前最强线上 `exp_082` **只差 4 张**
  - 全部是 `1 -> 0`
- 它和 `exp_083` 也只差 **4 张**
  - `0 -> 1`: `3`
  - `0 -> 2`: `1`
...
- `exp_085` 不是一个远离 `exp_082` 的新 submission 家族
- 它本质上是：
  - 相比 `exp_082`，收回了 4 个 Jesse rescue
  - 相比 `exp_083`，又放回了 4 个小分数 rescue
- 也就是说，它更像 **`082` 与 `083` 之间的 interpolation**，而不是新的机制突破

#### 为什么它没有形成明显新信号

- 对 `exp_085` 与 `exp_082` 差异样本逐条检查后发现：
  - 被撤回的 4 张，全是 Jesse rescue
  - 其中 3 张其实 `061_argmax == 1` 且 `067` 的 Jesse excess 很高
  - 它们没有被救回，不是因为 agreement-aware 反对，而是因为：
    - `score_061_final > 0.45`
    - 被 `low_conf_threshold` 这层 gate 挡住
- 这暴露出一个更深层的问题：
  - 当前 rescue-only 体系里，**真正主导边界的不是 agreement-aware margin**
  - 而仍然是 `061 == 0 且 score_061_final < low_conf_threshold` 这一层粗门槛
- 换句话说：
  - agreement-aware 确实改变了部分低分样本的接纳方式
  - 但它还没有强到足以重塑整个 rescue 集合

#### 对后续方向的启示

- `exp_085` 的价值在于：
  - 它证明 `agreement-aware` 是合理的细化轴
  - 但这条轴本身还不足以带来新的结构性提升
- 如果线上提交 `exp_085`，它更适合作为：
  - 一个**接近 `exp_082` 的低风险补充候选**
  - 用来验证那 4 个 Jesse rescue 是否真的是坏点
- 但无论 `exp_085` 线上涨跌，下一步都不该继续在这里细抠：
  - 不该再反复扫 `agree/disagree margin`
  - 不该继续做 `082/083/085` 这种纯 threshold 微调

#### 更值得继续的下一层

- 下一步应把关注点从“067 是否足够强”转到“061 为什么拒掉了它”：
  1. **`061 score-aware rescue`**
     - 显式利用 `score_061_base`
     - `score_061_neighbor`
     - `score_061_final`
     - `pred_061_argmax`
  2. **sample-wise meta-decider**
     - 不做全量 stacking
     - 只在 `061=0 且 067!=0` 的 rescue 子集上做微型决策器
  3. **围绕 `exp_082` 的局部修剪**
     - 重点看少量 Jesse rescue，而不是整体缩放整个 rescue 集合

#### 当前结论

- `exp_085` 是一次有价值的负半正结果：
  - 它没有证明 agreement-aware 已经是主突破
  - 但它把搜索空间进一步缩小了
- 最重要的认识是：
  - **当前系统的主杠杆已经不在“换哪套 margin”，而在“如何建模 `061` 的 reject 语义”**

### 从单脸裁剪问题到 `exp_086` 多脸候选原型（2026-04-03）

- 对 `train+val` 的 `raw / HAAR / MTCNN` 三联图人工检查后，已经可以非常明确地说：
  - 当前任务里存在一批**不适合“单脸裁剪 + 单标签监督”**的样本
  - 问题不只是检测不到脸，而是：
    - **多人图里裁错人**
    - **比较图里标签只对应其中一张脸**
    - **拼接图 / 坏图直接进入训练集**
...
- 手工裁训练/验证集当然能让 supervised set 更干净
- 但如果测试集仍然是自动检测：
  - 就会引入新的 train-test mismatch
- 更重要的是：
  - 测试集大概率也有同一类多人图 / 比较图问题
- 因此，手工裁剪更适合作为：
  - **数据审计工具**
  - 或少量“金标准问题样本”标注
- 而不应该作为最终方法主线
...
1. 对同一张图检测出全部候选脸  
2. 每张脸分别计算 Jesse / Mila 分数  
3. 图片级别按通道取最大证据  
4. 再做最终身份判定  

这正面解决了“裁错人”的问题，而不是赌检测器恰好选中了对的人。

#### `exp_086` 的设计

- 新配置：`exp_086_buffalo_l_detect_align_multiface_dual_verifier_lookalike`
- 与 `exp_067` 相比：
  - **gallery 不变**
  - **look-alike 阈值校准不变**
  - **只改 probe 侧**
- 新的 probe 侧规则：
  ...
  - 对每张候选脸分别算：
    - Jesse 相似度
    - Mila 相似度
  - Jesse 通道取全脸中的最高 Jesse 分
  - Mila 通道取全脸中的最高 Mila 分
  - 再用 dual verifier 在图片级做最终决策

#### `exp_086` 本地结果

- `val_accuracy_multiface = 1.0000`
- 相比 `exp_067` 风格 test 预测，改动 **140** 张：
  - `0 -> 1`: `66`
  - `0 -> 2`: `15`
  - `1 -> 0`: `28`
  - `2 -> 0`: `15`
...
- 这不是 `082/083/085` 那种“在同一套 rescue 阈值里继续抠 4 张、8 张”
- 它是一次**任务假设层级**的改动
- 更重要的是：
  - test 变化不是单向放宽
  - 也不是单向收紧
...
- `exp_086` 是一个**方法上合理、信息量高、值得提交**的候选
- 它是否立刻超过 `0.94768` 还不能先说死
- 但无论线上结果如何，它都会非常清楚地回答一个核心问题：
  - **真正的主瓶颈，是不是多人图里的目标人脸选择错误**

#### 后续如果 `exp_086` 有正反馈

- 下一步最自然的延伸不是回到手工裁剪
- 而是：
  1. 把 multi-face probe 接到更强的 `061/082` 主线上  
  2. 对明显坏样本（占位图、比较陷阱图）建立审计清单并做训练降权/剔除  
  3. 继续把系统从“单脸分类”升级到“图片级身份验证”框架

### `exp_086` labeled 审计结果（2026-04-03）

在 `exp_086` public score 达到 **`0.97026`** 之后，必须先验证它在 80 张 labeled 样本上是不是“方法上站得住”，而不是只在榜单上偶然奏效。
...
- `detect_align`
- dual verifier
- image-level multi-face max pooling

#### 审计结果

- 80 张 labeled 样本中，**79 / 80 正确**
- 审计准确率：**`0.9875`**
- 多脸分布：
  - `1 face`: `57`
  - `2 faces`: `17`
...
- 这张样本与此前人工观察一致：本体就是 **坏图 / 占位图**

这点很关键，因为它说明：

- 当前 `multiface` 自动化流程在正常样本上已经非常稳
- 剩下最明显的问题，不是“多人图仍经常选错脸”
- 而是**坏数据本身需要被显式识别和处理**

#### 用户点名样本核查

- `id=14`：多人比较陷阱图，当前预测正确为 `class1`
- `id=34`：检测出 `3` 张脸，最终仍正确给 `class1`
- `id=40`：检测出 `2` 张脸，最终正确给 `class2`
- `id=52`：检测出 `2` 张脸，最终正确给 `class1`
- `id=59`：检测出 `2` 张脸，最终保持 `other`
- `id=65`：坏图，仍失败
...
- 你之前怀疑“很多训练策略失败，根源其实是输入数据没处理好”
- 现在已经有很强证据支持这件事

更准确地说，问题不只是“crop 不够准”，而是：

- 单脸假设不成立
- 多人图里目标脸选择错误
- 少量坏图把监督直接污染了

因此接下来的主线应从“继续试训练配方”切换为：

1. 先建立 **自动化坏样本识别 / 标记**
2. 再把 **multi-face 自动选择** 接入更强模型主线
3. 最后才重新评估训练策略本身是否真的还需要大改

### `exp_087`：从“坏图处理”收敛到“原图精确重复继承标签”（2026-04-03）

在进一步拆解“坏图问题”后，发现一个更强、更干净的事实：

- test 中不只是存在坏图
- 而是存在与 train/val **原图完全一致** 的样本

进一步核查表明：

- test 中共有 **12 张**图片，其 raw `.npy` 数组与 train/val 某张带标签图片做 SHA-1 后完全一致
- 其中 `9` 张当前 `exp_086` 已经预测正确
- 剩下 `3` 张全部来自同一个坏图模板：
  - `train id=65 class=2`
  - 对应 test：
    - `id=537`
...
这 3 张在 `exp_086` 中都被判成 `0`，原因一致：

- 原图检测不到脸
- 走 `crop_fallback`
- 双通道分数都低

但它们与 `train id=65` 的 raw 数组是**字节级完全相同**，因此最合理的自动化规则不是“设计一个复杂坏图启发式”，而是：

> 当 test 原图与 train/val 某张带标签原图哈希完全一致时，直接继承该标签。
...
- 不依赖主观坏图判断
- 不靠 leaderboard 微操
- 本质上是在利用数据中明确存在的**精确重复**

#### `exp_087` 的具体效果

- 基于 `exp_086` submission 做 exact hash override
- 仅改动 `3` 张：
  - `0 -> 2`: `3`
- 没有碰其余 `1813` 张样本
...
`exp_087` 是一个非常低风险、方法上也站得住的增量实验。

它的重要性不只是那 3 张图本身，而是进一步说明：

- 这份数据里确实存在“输入级别”的结构问题
- 当前阶段如果只继续扫训练超参，会错过很多比“模型更深一点”更直接的增益
...
### `exp_088` / multiface-selected retraining（2026-04-03）

`exp_086/087` 已经证明，多脸自动选人本身是有效的；但那两条线仍然属于“推理层修正”。真正关键的问题是：

> 如果把“选对人脸”这件事前移到训练输入，`exp_061` 这种可训练分类器本身会不会变强？

因此本轮没有继续调融合阈值，而是做了一条受控重训线：

- 保持 `exp_061` 的主体训练协议不变
  - `AdaFace ViT`
  - `20 epoch`
  - 解冻最后 `2` 个 block
  - `CrossEntropy`
  - 同一套 neighborhood-aware 推理协议
- 只替换输入：
  - 从原图检测多张脸
  - `class1` 样本选 `class1` 分数最高的人脸
  - `class2` 样本选 `class2` 分数最高的人脸
  - `class0` 样本选最像目标类的 hardest negative 人脸
  - 明确坏图 `id=65` 从训练/验证集中剔除

#### 这条线的重要性

这不是又一条 leaderboard patch，而是第一次系统地验证：
...
- `data/processed/exp_088_multiface_selected_faces_112`
- `data/splits/exp_088_multiface_selected`

审计显示：

- train：`62` 张由 detect-align 正常选脸，`1` 张 fallback
- val：`16` 张全部 detect-align 正常选脸
- test：`1779` 张 detect-align 正常选脸，`37` 张 fallback

这说明：

- 在训练/验证集里，多脸自动选人已经几乎完全可用
- 坏图问题与“选错人”问题现在被更清楚地分开了
...
我不会把这个小验证集结果说得太满，但它至少说明：

- 一旦训练输入换成自动选中的正确人脸，`exp_061` 这条分类线并没有崩
- 相反，它在当前 split 上非常容易收敛
...
这和 `exp_082/086` 的大方向一致：主要收益仍然来自纠正过去过于保守的 `other` 拒判。
...
这说明 `exp_088` 不是简单复制 `exp_087`，而是在已有多脸推理优势之上，进一步把分类边界向 `class1` 方向推开了一些。

#### 当前判断

`exp_088` 是一条非常高信息量的实验：

- 如果线上继续涨，就基本可以确认：
  - 过去很多训练策略没有兑现，不是因为 backbone 不行，而是因为输入人脸长期选错了
- 如果线上不涨，也依然有价值，因为这能把“多脸选人更适合推理修正，而不一定适合直接重训”这个边界划清

换句话说，`exp_088` 的意义不是再做一次小修补，而是第一次直接检验：

> 多脸自动选人，能否从推理技巧升级成训练协议。

#### 线上结果回填

`exp_088` 已提交 public leaderboard，分数为：

- `0.98403`

这一步直接把前面的方法学判断坐实了：

- `exp_086/087` 证明了“多脸自动选人”在推理层有效
- `exp_088` 进一步证明：把这件事前移到训练输入，收益更大

因此现在最重要的结论不是“又多涨了一次分”，而是：

> 当前任务的真正突破，不是某个新的 loss 或门限，而是修正了“图里该看哪张脸”这个输入定义。

这也解释了为什么此前很多训练实验没有持续超过后处理线：

- 当训练时长期喂给模型错误的人脸 crop
- backbone、学习率、解冻策略再怎么改，也会被错误输入上限压住

而 `exp_088 = 0.98403` 说明：

- 一旦输入协议改对，原本稳定但保守的分类主线可以直接跃升为当前最强方案
- 后续实验的默认起点应该是 `multiface-selected`，不再是 `HAAR` 单脸 crop

### `exp_089` / class0 top-2 hard negatives（2026-04-04）

在 `exp_088 = 0.98403` 之后，最自然的下一问是：

> 当输入脸已经选对以后，剩余提升是否主要来自更强的 `class0` 负样本建模？

因此本轮做了一个非常受控的增量：

- 保持 `exp_088` 的 backbone、训练超参、推理协议不变
- 只改 train split 中 `class0` 的输入展开方式
- 从“每张 `class0` 图只保留 1 张 hardest negative”
- 改成“保留前 2 张 hardest negatives”

#### 实际结果

这轮改动比预想更窄：

- 只有 `3` 个 `class0` 训练样本真的存在第二张足够危险的人脸
  - `id=41`
  - `id=59`
  - `id=63`
- 因此 train 只从 `63` 张扩到 `66` 张

训练完成后：

- `best_val_acc = 1.0`

但更关键的是 submission 对比：

- `exp_089` 与 `exp_088` **逐行完全一致**
- `num_diff = 0`
...
`exp_089` 的价值不在于涨分，而在于排除了一个很合理但当前并不成立的假设：

> 当前主线的下一增益，并不是简单给 `class0` 再加一张 hardest negative。
...
- 更强训练强度
  - 例如在同一 `multiface-selected` 输入协议下，解冻更多层或延长训练
- 或继续减少 `crop_fallback`
  - 因为这两条现在比“再加一张负脸”更可能改变最终 submission

### `exp_090` / last4block stronger finetune（2026-04-04）

在 `exp_089` 排除了 `class0 top-k negatives` 之后，本轮转向了最像“真正单体模型强化”的下一步：

- 保持 `exp_088` 的输入协议不变
- 保持 CE loss、不改 threshold、不改推理协议
- 只把 ViT backbone 的可学习深度从最后 `2` 个 block 提升到最后 `4` 个 block
- 同时把 backbone learning rate 从 `1e-5` 压到 `5e-6`
...
- 当前 `exp_088` 的成功，不是因为“last2block 恰好太弱”
- 也不是简单再放开更多 backbone，就能自然多捡几分

换句话说，当前主线的瓶颈已经不是“模型还没学够”，至少不是这种最直接的加深 finetune 方式。

#### 对下一步方向的影响

现在已经连续排除了两种最自然的增量：

- `exp_089`：更多 `class0` hardest negatives
- `exp_090`：更深层 finetune

两者都没有改变最终 submission。
...
- 改训练目标，而不只是改 finetune 深度
- 或继续处理剩余 `crop_fallback`

### `exp_091` / `exp_092` / val-loss rerun of champion line（2026-04-04）

这轮实验不是新想法，而是对训练 protocol 的纠偏。

用户指出的批评是对的：

- 当前 val 只有 `16` 张
- `val_acc` 很早就会以 `1/16` 的粗粒度饱和到 `1.0`
- 但这并不代表模型已经停止改进
- 之前 `exp_088` / `exp_090` 的日志里，`val_loss` 在 `val_acc=1.0` 之后还在持续下降
...
- 都改成：
  ...
  - `monitor_mode = min`
  - `max_epochs = 50`
  - 更宽松的 early stopping patience

#### 实际发生了什么

这次最重要的第一层结论，是用户对 protocol 的批评被**正面验证**了：

- 两个实验都没有再在不到 `10 epoch` 时结束
- 两者都训练到了接近 `50 epoch`
- `val_acc` 很早就到 `1.0`
- 但 `val_loss` 继续稳步下降到很低

也就是说：

- 之前用 `val_acc` 监控冠军主线，确实不严谨
- 这个问题不是感受层面的担忧，而是被训练曲线直接证实的

#### 但第二层、也是更关键的结论

即便在更合理的训练协议下：

- `exp_091` 与 `exp_088` 的 submission **逐行完全一致**
- `exp_092` 与 `exp_088` 的 submission **逐行完全一致**

也就是说，之前那两个实验的“训练过程”确实没有被公平地跑完，但把它们公平跑完之后：

- `exp_088` 这条冠军主线没有被推翻
- `exp_090` 的 stronger finetune 也仍然没有改变 test 侧最终决策

#### 这轮实验真正留下的工程结论

这轮不是没有收获，恰恰相反，信息量很大：

1. 以后这条主线**必须默认用 `val_loss` 做选模与早停监控**，不能再回到 `val_acc`
2. `exp_088 = 0.98403` 的稳定性现在更可信了，因为它经受住了 protocol 纠偏后的复查
3. `exp_090` 没有收益这件事，现在也能更放心地成立，不再只是“可能被早停误伤”的暂时判断
