...
- **2026-04-02 学术结论（已被修正）：** 曾认为 `0.927` 是 AdaFace 单模型 + neighborhood-aware 推理的”铁底板极限”。
- **2026-04-03 修正：** 引入 insightface `buffalo_l`（ArcFace ResNet50@WebFace600K）作为完全不同的模型家族，实现双独立验证器 + look-alike 分簇阈值校准。`exp_067 = 0.92676`，几乎追平 `exp_061 = 0.92731`，但两者预测差异达 **145 张**——说明两个模型犯的错误高度互补。此前”0.927 是铁底板”的结论被证伪：它只是单一 AdaFace embedding + 固定推理协议的上限，不是任务上限。
- 当前最有价值的下一步方向：**异构 embedding 融合**（ArcFace + AdaFace），利用 145 张互补错误样本来突破 0.93。
