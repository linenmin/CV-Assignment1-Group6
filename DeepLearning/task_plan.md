...
- `exp_033` 已给出对这条线的第一轮回答：`ViT fine-tune` 不是完全无效，但当前收益主要表现为更松的接收；结合 `exp_032` 的线上胜出，下一步不宜继续单模型深挖 `ViT`，而应优先尝试 `IR101 + ViT` 的异构融合，并把 `0.55` 作为更可信的默认阈值。
- `exp_034/035` 已说明“直接做线性 mean fusion”不是当前最优策略；若继续走异构融合，下一步应优先考虑更偏 `ViT` 的策略或非线性 gating，而不是继续扫更多线性平均权重。
